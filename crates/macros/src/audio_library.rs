  ...
  let nested_from_arms = prefix_infos.iter().map(|info| {
    ...
    match def {
      AudioDefInfo::Once { asset_path } => quote! {
        retval.insert(#enum_name::#variant_name, crate::audio_engine::AudioDef::Once(asset_server.load_acquire(#asset_path, guard.clone())));
      },
      AudioDefInfo::Loop { asset_path } => quote! {
        retval.insert(#enum_name::#variant_name, crate::audio_engine::AudioDef::Looped(asset_server.load_acquire(#asset_path, guard.clone())));
      },
      AudioDefInfo::IntroLoop {
        start_path,
        main_path,
      } => quote! {
        retval.insert(#enum_name::#variant_name, crate::audio_engine::AudioDef::IntroLooped { intro: asset_server.load_acquire(#start_path, guard.clone()), main: asset_server.load_acquire(#main_path, guard.clone()) });
      },
    }
  });

  Ok(quote! {
      impl #impl_generics crate::audio_engine::AudioLibrary for #enum_name #type_generics #where_clause {
        fn load_all(asset_server: &bevy::asset::AssetServer, guard: utils::assets::AssetBarrierGuard) -> bevy::platform::collections::HashMap<Self, crate::audio_engine::AudioDef> {
