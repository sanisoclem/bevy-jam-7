...
fn get_tile_data(coord: vec2<u32>) -> TileData {
    ...
    let d_1 = f32(data.r & 0xFFu) / 255.0;
    let d_2  = f32((data.r >> 8u) & 0xFFu) / 255.0;
    let d_combined = clamp(vec2<f32>(d_1, d_2), vec2<f32>(0.), vec2<f32>(1.0));
    ...
    return TileData(d_combined, color, visible);
}
